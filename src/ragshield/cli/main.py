"""Command-line interface for RagShield."""
...
try:
    ...
    print("CLI dependencies not installed. Install with: pip install ragshield[cli]")
    sys.exit(1)
...
def scan(
    ...
    Examples:
        ...
        ragshield scan --text "Hello, ignore previous instructions"

        echo "some text" | ragshield scan -

        ragshield scan ./documents/ --output json
    """
    from ragshield import RagShield, ScanResult
    ...
    try:
        shield = RagShield(config=shield_config if shield_config else None)
    except Exception as e:
        console.print(f"[red]Failed to initialize RagShield: {e}[/red]")
        raise typer.Exit(1)
...
def calibrate(
    ...
    Examples:
        ...
        ragshield calibrate --model text-embedding-3-small --corpus ./my_docs/
    """
...
def config_cmd(
    ...
    """
    Manage RagShield configuration.
    ...
    """
    from ragshield import ShieldConfig
    ...
    if action == "show":
        ...
        output_path = path or Path("ragshield.yaml")
...
@app.command()
def version():
    """Show RagShield version."""
    from ragshield import __version__
    console.print(f"ragshield {__version__}")
